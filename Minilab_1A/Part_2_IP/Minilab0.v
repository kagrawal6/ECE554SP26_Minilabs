...
reg [DATA_WIDTH*3-1:0] accumulator;  // MAC accumulator
...
adder_ip adder (
    .dataa(accumulator),
    .datab({8'b0, mult_result}),  // Zero-extend 16-bit to 24-bit
    .result(add_result)
);
  ...
  if (~rst_n) begin
    ...
    case(state)
      ...
      begin
        ...
        // Transition when FIFOs are empty
        if (empty) begin
          state <= DONE;
        end
      end
