module matrix_vector_multiplier #(
    ...
    state_t state;
    logic [4:0] exec_cnt;       // Execution cycle counter (5-bit)
    logic       en_src;         // Enable source for FIFO reads
    ...
    //  With registered FIFO output, en_delay has DIM stages:
    //    en_delay[0] = en_src delayed 1 → aligns with FIFO output → feeds MAC[0]
    //    en_delay[i] = en_delay[i-1] delayed 1              → feeds MAC[i]
    //  b_delay has DIM-1 stages (same as before):
    //    b_delay[0]  = b_fifo_out delayed 1                 → feeds MAC[1]
    //    b_delay[i]  = b_delay[i-1] delayed 1               → feeds MAC[i+1]
    // -------------------------------------------------------
    logic [DIM-1:0]        en_delay;                    // 8 pipeline stages
    ...
    // =========================================================
    //  FIFO read enables (unchanged — read 1 cycle before data needed)
    //    A[0] and B read with en_src
    //    A[i] (i>=1) reads with en_delay[i-1]
    // =========================================================
    ...
    //
    //  en_delay[0] = en_src delayed 1 cycle → feeds MAC[0] (aligned with registered FIFO out)
    //  en_delay[i] = en_delay[i-1] delayed 1 → feeds MAC[i]
    //
    //  b_delay[0]  = b_fifo_out delayed 1   → feeds MAC[1]
    //  b_delay[i]  = b_delay[i-1] delayed 1 → feeds MAC[i+1]
    ...
    // =========================================================
    //  Instantiate 8 MACs
    //  With registered FIFO output (+1 cycle latency):
    //    MAC[0]: En = en_delay[0], Bin = b_fifo_out   (both aligned to FIFO output)
    //    MAC[i]: En = en_delay[i], Bin = b_delay[i-1]
    // =========================================================
    generate
    ...
    //  EXEC:    en_src=1 for 8 cycles (reads FIFOs),
    //           registered FIFO output adds 1 cycle latency,
    //           pipeline propagates En/B to MACs 1-7,
    //           3-stage MAC adds 3 cycles,
    //           total 18 cycles (exec_cnt 0-17) for all MACs
        ...
        if (~rst_n) begin
            ...
            case (state)
                IDLE: begin
                    ...
                    if (fetch_done) begin
                        state  <= EXEC;
                        en_src <= 1'b1;  // Start reading FIFOs
                    end
                end

                EXEC: begin
                    exec_cnt <= exec_cnt + 5'd1;
                    if (exec_cnt == 5'd7)
                        en_src <= 1'b0;      // Stop reading after 8 values
                    if (exec_cnt == 5'd17)
